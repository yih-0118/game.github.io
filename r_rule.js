...
// 新增: 提交分數到Google Form的函數
function submitScore(name, score) {
    return $.ajax({
        url: "https://docs.google.com/forms/d/e/1FAIpQLSebkHFGbc3B7xatN22mKTGeyA-H1MdlEzTYsMk5bXJ9iKb2TA/formResponse",
        crossDomain: true,
        data: {
            "entry.650930505": name,
            "entry.175692817": score
        },
        type: "POST",
        dataType: "JSON"
    });
...
function handleGameOver() {
    const name = prompt("遊戲結束！請輸入您的名字：");
    if (name) {
...
pause_button.addEventListener("click", () => {
    if (is_paused) { // 如果已暫停
        ...
        drop(); // 繼續遊戲
        pause_button.innerHTML = '<i class="fas fa-pause"></i>'; // 更新按鈕文字
    } else { // 如果未暫停
        is_paused = true; // 設置暫停
        pause_button.innerHTML = '<i class="fas fa-play"></i>'; // 更新按鈕文字
    }
});
...
function drop() {
    ...
    if (delta > 125) { // 如果時間差大於250毫秒，一秒下降4格
...
const SWIPE_THRESHOLD = 20; // 滑動判定的最小距離
